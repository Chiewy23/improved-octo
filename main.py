...
class TextEditor:
    ...
    def __init__(self, _root):
        ...
        # ##### TITLE BAR #####
        self.title_bar = Label(self.root, textvariable=self.title, font=("times new roman", 15, "bold"), bd=2,
                               relief=GROOVE)
        self.title_bar.pack(side=TOP, fill=BOTH)
        self.set_title()

        # ##### STATUS BAR #####
        self.status_bar = Label(self.root, textvariable=self.status, font=("times new roman", 15, "bold"), bd=2, relief=GROOVE)
        self.status_bar.pack(side=BOTTOM, fill=BOTH)
        self.status.set("Welcome To Text Editor")

        # ##### MENU BAR #####
        self.menu_bar = Menu(self.root, font=("times new roman", 15, "bold"), activebackground="skyblue")
        # Configuring menubar on root window
        self.root.config(menu=self.menu_bar)

        # ##### FILE MENU #####
        self.file_menu = Menu(self.menu_bar, font=("times new roman", 12, "bold"), activebackground="skyblue", tearoff=0)
        ...
        # ##### EDIT MENU #####
        self.edit_menu = Menu(self.menu_bar, font=("times new roman", 12, "bold"), activebackground="skyblue", tearoff=0)
        ...
        # ##### HELP MENU #####
        self.help_menu = Menu(self.menu_bar, font=("times new roman", 12, "bold"), activebackground="skyblue", tearoff=0)
        self.help_menu.add_command(label="About", command=info_about)
        self.menu_bar.add_cascade(label="Help", menu=self.help_menu)

        # ##### SCROLLBAR #####
        scroll_y = Scrollbar(self.root, orient=VERTICAL)
        self.text_area = Text(self.root, yscrollcommand=scroll_y.set, font=("times new roman", 15, "bold"), state="normal", relief=GROOVE)
    ...
    def save_as_file(self, *args):
        try:
            untitled_file = filedialog.asksaveasfilename(title="Save file As", defaultextension=".txt", initialfile="Untitled.txt", filetypes=(("All Files", "*.*"), ("Text Files", "*.txt"), ("Python Files", "*.py")))
            data = self.text_area.get("1.0", END)
